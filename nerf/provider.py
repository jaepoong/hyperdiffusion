...
def nerf_matrix_to_ngp(pose, scale=0.33, offset=[0, 0, 0]):
    ...
    new_pose = np.array([
        [pose[1, 0], -pose[1, 1], -pose[1, 2], pose[1, 3] * scale + offset[0]], # y->x
        [pose[2, 0], -pose[2, 1], -pose[2, 2], pose[2, 3] * scale + offset[1]], # z->X
        [pose[0, 0], -pose[0, 1], -pose[0, 2], pose[0, 3] * scale + offset[2]], # x->z
        [0, 0, 0, 1],
    ], dtype=np.float32)
    return new_pose
...
def line_poses(size, device, radius=1, phi_range=[0, 2*np.pi]):
    ...
    thetas = torch.tensor([np.pi/3]*size).to(device)
    phis = torch.linspace(0,1,size, device=device) * (phi_range[1] - phi_range[0]) + phi_range[0]
...
class NeRFDataset:
    def __init__(self, opt, device, type='train', downscale=1, n_test=10):
        ...
        self.training = self.type in ['train', 'all', 'trainval','testval']
        ...
        if self.mode == 'colmap' and type == 'test':
            ...
            f0, f1 = np.random.choice(frames, 2, replace=False) # 두개 test셋 사용.
