...
class Trainer(object):
    def __init__(self, 
                 ...
                 use_checkpoint="best", # which ckpt to use at init time
    ...
    def train(self, train_loader, valid_loader, max_epochs):
        ...
        for epoch in range(self.epoch + 1, max_epochs + 1):
            ...
            # change : 주석 없에면 최종 결과 두개 저장됨.
            #if self.workspace is not None and self.local_rank == 0:
            #    self.save_checkpoint(full=True, best=False)

            if self.epoch % self.eval_interval == 0: # evry 50 epoch evaluate one metric
                self.evaluate_one_epoch(valid_loader)
                self.save_checkpoint(full=False, best=True) # best result만 저장 할거임.
    ...
    def test(self, loader, save_path=None, name=None, write_video=True):
        ...
        if write_video:
            ...
            imageio.mimwrite(os.path.join(save_path, f'{name}_rgb.mp4'), all_preds, fps=5, quality=8, macro_block_size=1)
            imageio.mimwrite(os.path.join(save_path, f'{name}_depth.mp4'), all_preds_depth, fps=5, quality=8, macro_block_size=1)
    ...
    def save_checkpoint(self, name=None, full=False, best=False, remove_old=True):
        ...
        if not best:
            ...
            if len(self.stats["results"]) > 0: # best statment saving.
                if self.stats["best_result"] is None or self.stats["results"][-1] < self.stats["best_result"]: # if best result.
